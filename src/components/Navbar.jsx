...
const Navbar = () => {
  ...
  // Organize categories into column groups
  useEffect(() => {
  ...
  const categoryColumns = () => {
    const columns = [];
    const columnsCount = 3; // Adjust based on your layout
        ...
        <div className="flex justify-between items-center">
          ...
          <div className="hidden lg:flex flex-1 justify-center">
            <div className="flex space-x-6 items-center">
              <Link to="/" className="font-medium text-sm text-gray-800 hover:text-yellow-400 transition-all duration-300 relative group py-2">
                Home
                <span className="absolute -bottom-1 left-0 w-0 h-0.5 bg-yellow-400 transition-all duration-300 group-hover:w-full"></span>
              </Link>
              ...
              <div className="relative group">
                ...
                {/* Products Dropdown - Updated with all categories */}
                {isProductsOpen && (
                  <div className="absolute left-1/2 transform -translate-x-1/2 mt-2 w-screen max-w-4xl bg-white shadow-xl rounded-lg z-10 overflow-hidden transition-all duration-300 opacity-100 border border-gray-100">
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 p-6">
                          ...
                          {column.map((category, catIndex) => (
                            <div key={catIndex} className={catIndex > 0 ? "mt-6" : ""}>
                              ...
                              <Link 
                                ...
                                className="text-xs font-medium text-black hover:text-yellow-400 mt-2 inline-block hover:underline"
                      ...
                      {/* Product Catalog Download - Fixed in Last Column */}
      ...
      {mobileMenuOpen && (
        <div className="lg:hidden bg-white border-t border-gray-100 max-h-screen overflow-y-auto transition-all duration-300">
          <div className="container max-w-5xl mx-auto py-3 px-8 space-y-3">
            <Link to="/" className="block font-medium text-sm text-gray-800 hover:text-yellow-400 py-1.5 border-b border-gray-100">
              Home
            </Link>
            <Link to="/about-us" className="block font-medium text-sm text-gray-800 hover:text-yellow-400 py-1.5 border-b border-gray-100">
              About Us
            </Link>
                ...
                <div className="mt-2 pl-3 space-y-2">
                  {categories.map((category, index) => (
                    <div key={index} className="py-1">
                      <h3 className="font-semibold text-xs text-yellow-400">{category}</h3>
                      <Link 
                        to={`/category/${categoryToSlug(category)}`} 
                        className="mt-1 block text-xs text-gray-600 hover:text-yellow-400 transition-colors duration-300"
            ...
            <Link to="/services" className="block font-medium text-sm text-gray-800 hover:text-yellow-400 py-1.5 border-b border-gray-100">
              Services
            </Link>
            <Link to="/industries" className="block font-medium text-sm text-gray-800 hover:text-yellow-400 py-1.5 border-b border-gray-100">
              Industries
            </Link>
